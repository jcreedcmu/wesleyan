...
export function comps(n: number): number[][] {
  if (n <= 0) return [[]];
  let rv: number[][] = [];
  for (let i = 1; i <= n; i++) {
...
export function epretty(e: Exp): string {
...
export function spretty(e: Exp): string {
  if (Object.keys(e).every(k => e[k] == 0)) {
    return '0';
  }
...
export function plus(t1: Exp, t2: Exp): Exp {
  const rv: Exp = {};
...
export function plusa(...args: Exp[]): Exp {
...
export function sep(s: number, e: Exp): Exp {
  const rv: Exp = {};
  for (const t of Object.keys(e)) {
    rv[t] = s * e[t];
  }
...
export function sub(t1: Exp, t2: Exp): Exp {
  return plus(t1, sep(-1, t2));
}

type TreeCombiner = (tr1: Tree, tr2: Tree) => Tree;

export function defaultTreeCombiner(tr1: Tree, tr2: Tree): Tree {
  return [...tr1, ...tr2];
}

export function lieTreeCombiner(tr1: Tree, tr2: Tree): Tree {
  return [[tr1, tr2]];
}

// tree-expression product, f to combine trees
export function tep(tr: Tree, e: Exp, f: TreeCombiner): Exp {
  const rv: Exp = {};
...
export function prod(e1: Exp, e2: Exp, f: TreeCombiner = defaultTreeCombiner): Exp {
  let rv: Exp = {};
...
export function proda(...args: Exp[]): Exp {
  return args.reduce((x, y) => prod(x, y));
}

export function lie(e1: Exp, e2: Exp): Exp {
  return prod(e1, e2, lieTreeCombiner);
}

export function glie(n1: number, n2: number): Exp {
...
// (n!/k!) G_{λ₁}⋯G_{λₖ}
export function target(n: number): Exp {
